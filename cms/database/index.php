 ...
 * This file is part of the source code. And used phpLiteAdmin 1.9.8.2 for database management.
 */
...
if (!is_login()) {
    ...
    if ($type_db != 'phpSQLite3') {
        header('Location: ' . $config_db['QuerySQL']['pma']);
        exit;
    }
}

//directory relative to this file to search for databases (if false, manually list databases in the $databases variable)
$directory = $config_db['phpSQLite3']['directory'];
...
$databases = array(
	array(
	    'name' => $config_db['phpSQLite3']['name'],
...
$save_new_db = [
    'name' => $config_db['phpSQLite3']['name'],
    'path' => $config_db['phpSQLite3']['directory'] . $config_db['phpSQLite3']['path'],
    'type' => 3
];
...
$lang = array(
	"direction" => "LTR",
	"date_format" => 'H:i:s, d/m/Y',  // see http://php.net/manual/en/function.date.php for what the letters stand for
	"ver" => "version",
	"for" => "for",
	"to" => "to",
	"go" => "Go",
	...
	"no_db" => "Chào mừng đến với %s. Khi bạn nhìn thấy thông báo này tức là cơ sở dữ liệu của trang web bạn chưa được khởi tạo.",
...
if(count($databases)==0) // the database array is empty, offer to create a new database
{
	...
	if($directory!==false && is_writable($directory))
	{
		...
		unlink($directory . DIRECTORY_SEPARATOR . 'database.sqlite');
		$td = new Database($save_new_db);
		$td->query("VACUUM");
...
echo "<div id='leftNav'>";
echo "<h1><a href='/cms'>";
echo "<span id='logo'>DOREW</span>";
echo "</a></h1>";
echo "<div id='headerlinks'>";
echo "<a href='/database'>Database</a> | <a href='https://" . $http_host . "' title='Go to website'>" . $http_host . "</a>";
echo "</div>";

//- HTML: database list

echo "<fieldset style='margin:15px;'>";
echo "<legend><b>List</b></legend>";
/*
echo "<legend>";
...
<script>
    var w = screen.width;
    var cookie_name = 'screen_width';
    document.cookie = cookie_name + "=" + w + "; path=/";
</script>
<?php
$screen_width = $_COOKIE['screen_width'];
if ($screen_width < 767) {
    $textarea_cols = '23';
    $scrollTableStart = "<div class='scrollTable'><table border='0' class='viewTable'>";
    $scrollTableEnd = "</table></div>";
    echo '</tr><tr>';
} else {
    $textarea_cols = '50';
    $scrollTableStart = "<table border='0' class='viewTable'>";
    $scrollTableEnd = "</table>";
}
echo '<td valign="top" id="main_column" class="right_td" style="padding:9px 2px 9px 9px;">';
...
if(isset($_GET['action']) && !isset($_GET['confirm']))
{
	...
	//- Table actions
		...
		case "table_sql":
			...
			echo "<textarea class='textarea' name='queryval' id='queryval' rows='8' cols='".$textarea_cols."'>".htmlencode($queryStr)."</textarea>";
			...
			if(!isset($_GET['search']))
			{
				...
				for($i=0; $i<sizeof($tableInfo); $i++)
				{
					...
					if($typeAffinity=="INTEGER" || $typeAffinity=="REAL" || $typeAffinity=="NUMERIC")
						...
						echo "<textarea id='field_".$i."_value' name='field_".$i."_value' rows='1' cols='".$textarea_cols."'>".htmlencode($value)."</textarea>";
					echo "</td>";
					echo "</tr>";
				}
				echo "<tr>";
			...
			for($j=0; $j<$num; $j++)
			{
				...
				for($i=0; $i<sizeof($tableInfo); $i++)
				{
					...
					elseif(preg_match('/^BLOB/', $type))
						...
						echo "<textarea id='row_".$j."_field_".$i."_value' name='".$j.":".$i."' rows='5' cols='".$textarea_cols."' onclick='notNull(\"row_".$j."_field_".$i."_null\");' onblur='changeIgnore(this, \"row_".$j."_ignore\");'>".$value."</textarea>";
					echo "</td>";
					echo "</tr>";
				}
				echo "<tr>";
					...
					for($j=0; $j<sizeof($pks); $j++)
					{
						...
						for($i=0; $i<sizeof($tableInfo); $i++)
						{
							...
							elseif(preg_match('/^BLOB/', $type))
							{
								...
								echo "<textarea id='row_".$j."_field_".$i."_value' name='".$i."[]' rows='1' cols='".$textarea_cols."' class='".htmlencode($field)."_textarea' onblur='changeIgnore(this, \"".$j."\", \"row_".$j."_field_".$i."_null\")'>".htmlencode($value)."</textarea>";
							echo "</td>";
							echo "</tr>";
						}
						echo "<tr>";
	...
	//- Column actions
		...
		case "trigger_create":
			...
			else
			{
				...
				echo "<textarea name='whenexpression' class='textarea' rows='8' cols='".$textarea_cols."'></textarea>";
				echo "<br/><br/>";
				echo $lang['trigger_step'].":<br/>";
				echo "<textarea name='triggersteps' class='textarea' rows='8' cols='".$textarea_cols."'></textarea>";
				echo "</fieldset><br/><br/>";
...
if(!$target_table && !isset($_GET['confirm']) && (!isset($_GET['action']) || (isset($_GET['action']) && $_GET['action']!="table_create"))) //the absence of these fields means we are viewing the database homepage
{
  //- Switch on $view (actually a series of if-else)
	...
	if($view=="structure")
	{
		...
		echo "<b>".$lang['sqlite_v']."</b>: ".$db->getSQLiteVersion()."<br/><br/>";
		...
		else
		{
			...
			echo "<td class='tdheader' colspan='8'>".$lang['act']."</td>";
			...
			foreach($tables as $tableName => $tableType)
			{
				...
				{
					$skippedTables = true;
					$records = $params->getLink(array('forceCount'=>'1'), '0');
				}
				else
			...
			echo "<tr>";
			echo "<td class='tdheader' colspan='10'>".sizeof($tables)." ".$lang['total']."</td>";
		...
		{
			echo "<fieldset>";
			echo "<legend><b>".$lang['create_tbl_db']."</b></legend>";
...
class GetParameters
{
    ...
    {
        unset($this->_fields['database']);
        global $getURL_sqlite3;
        $arg_sep = ($html ? '&amp;' : '&');
        $this_query = array_merge($this->_fields, $assoc);
        $query = http_build_query($this_query, '', $arg_sep);
        $url = $prefix . $query;
        return str_replace('?database=' . $getURL_sqlite3 . '&amp;', '?', $url);
    }
